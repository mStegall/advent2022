...
pub fn main() {
    ...
    let mut field = (0..(*deep + 2))
        .map(|_| (left..(right + 1)).map(|_| State::Air).collect::<Vec<_>>())
        .collect::<Vec<_>>();

    field.push((left..(right + 1)).map(|_| State::Rock).collect::<Vec<_>>());
