...
pub fn main() {
    ...
    states_seen.insert((vec![], "AA".to_string(), 26), 0);

    let mut states_available: VecDeque<(Vec<String>, String, usize, usize)> = VecDeque::new();
    states_available.push_front((vec![], "AA".to_string(), 26, 0));
    ...
    for ((v1, _, _), p) in states_seen.iter() {
        let mut v = v1.clone();
        v.sort();
        best.entry(v)
            .and_modify(|p1| {
    ...
    let max = best
        ...
        .map(|(v1, p1)| {
            best.iter()
                .filter_map(|(v2, p2)| {
                    if v2.iter().all(|s| !v1.contains(s)) {
                        Some(p1 + p2)
                    } else {
                        None
                    }
